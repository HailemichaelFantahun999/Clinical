...
export default function PublicTrials() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!text) return trials
    return trials.filter((trial) => {
      const haystack = [
        trial.data?.title,
        trial.data?.briefSummary,
        trial.data?.diseases,
        trial.data?.contactInstitution,
        trial.data?.recruitmentCentreCity
      ]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 lg:px-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <label className="w-full max-w-xs text-xs">
          <span className="mb-1 block font-medium text-slate-700">Search trials</span>
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="field-input"
            placeholder="Search by title, disease, city, or institution"
          />
        </label>
      </div>
      <div className="mt-5 space-y-3">
        {!filtered.length && (
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-4 py-6 text-sm text-slate-500 shadow-soft">
            No approved trials match this search yet.
          </div>
        )}
        {filtered.map((t) => (
          <article
          ...
          >
            ...
            <dl className="mt-3 grid gap-2 text-[11px] text-slate-500 sm:grid-cols-4">
              <div>
                <dt className="font-semibold text-slate-600">Trial Design</dt>
                <dd>{t.data.trialDesign || '—'}</dd>
              </div>
              <div>
                <dt className="font-semibold text-slate-600">Disease</dt>
                <dd>{t.data.diseases || '—'}</dd>
              </div>
              <div>
