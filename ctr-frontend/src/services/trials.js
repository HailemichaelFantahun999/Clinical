...
export async function updateTrial({ trialId, payload }) {
  try {
    const { data } = await api.patch(`/api/trials/${encodeURIComponent(trialId)}`, { payload })
    return data.trial
  } catch (err) {
    throw new Error(errorMessage(err, 'Unable to update trial.'))
  }
}
...
export async function adminPendingTrialQueue() {
  try {
    const { data } = await api.get('/api/admin/trials/pending-queue')
    return data.trials || []
  } catch (err) {
    throw new Error(errorMessage(err, 'Unable to load pending trials.'))
  }
}

export async function adminAssignReviewer({ trialId, reviewerId }) {
  try {
    const { data } = await api.post(`/api/admin/trials/${encodeURIComponent(trialId)}/assign-reviewer`, {
      reviewerId
    })
    return data.trial
  } catch (err) {
    throw new Error(errorMessage(err, 'Unable to assign reviewer.'))
  }
}
