...
const navByRole = {
  admin: [
    ...
    { to: '/dashboard/admin/assign-trials', label: 'Assign trials' },
    ...
    <div className="flex min-h-screen bg-slate-50">
      <aside className="hidden w-64 flex-col border-r border-slate-200 bg-white/80 px-3 py-4 shadow-soft lg:flex">
        <Link to="/" className="mb-6 flex items-center gap-2 px-2">
          ...
          <div className="text-xs font-semibold text-slate-900">ECTR</div>
        </Link>
        <div className="mb-4 px-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">Navigation</div>
        <nav className="flex flex-1 flex-col gap-1">
          {items.map((item) => {
            const itemUrl = new URL(item.to, 'https://example.com')
            const currentUrl = new URL(`${location.pathname}${location.search}`, 'https://example.com')
            const isActive =
              itemUrl.pathname === '/dashboard/reviewer'
                ? currentUrl.pathname === itemUrl.pathname && currentUrl.search === itemUrl.search
        ...
        <header className="flex items-center justify-between border-b border-slate-200 bg-white/80 px-4 py-3 shadow-sm lg:px-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-500">Ethiopian Clinical Trial Registry</div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Link
              to="/trials"
              className="hidden rounded-full border border-slate-200 px-3 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 sm:inline-flex"
            >
              Search trials
            </Link>
            <button
